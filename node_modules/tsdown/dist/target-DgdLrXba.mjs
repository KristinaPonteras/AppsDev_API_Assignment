...
import { c as resolveComma, d as toArray, i as importWithError, t as styleText } from "./style-DiMP8pYv.mjs";
...
import { findMinimumForRange, isGreaterOrEqual, normalize } from "verkit";
...
async function buildSingleExe(config, bundledFile, outputFile, executable, target) {
	...
	if (stat) {
		...
		config.logger.info(config.nameLabel, styleText.bold(path.relative(config.cwd, outputPath)), ` ${styleText.dim(sizeText)}`);
	}
	config.logger.success(config.nameLabel, `Built executable: ${styleText.red(path.relative(config.cwd, outputPath))}`, styleText.dim(`(${Math.round(performance.now() - t)}ms)`));
}
//#endregion
