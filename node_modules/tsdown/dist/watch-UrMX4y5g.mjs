...
import { c as resolveComma, d as toArray, o as noop, t as styleText$1 } from "./style-DiMP8pYv.mjs";
import { s as prettyFormat } from "./logger-D9XFzQl9.mjs";
...
async function outputReport(config, chunks, outDir, cjsDts, isDualFormat) {
	...
	if (!options.summary) for (const size of sizes) {
		const filenameColor = size.dts ? styleText$1.green : noop;
		const filename = path.normalize(size.filename);
		config.logger.info(config.nameLabel, formatLabel, styleText$1.dim(outDir + path.sep) + filenameColor(size.isEntry ? styleText$1.bold(filename) : filename), ` `.repeat(filenameLength - size.filename.length), styleText$1.dim(size.rawText), options.gzip && size.gzipText && styleText$1.dim(`│ gzip: ${size.gzipText}`), options.brotli && size.brotliText && styleText$1.dim(`│ brotli: ${size.brotliText}`));
	}
		...
		async writeBundle(options, bundle) {
			for (const chunk of Object.values(bundle)) {
				...
				if (await fsExists(filepath)) {
					logger.info(nameLabel, isDualFormat && prettyFormat(options.format), `Granting execute permission to ${styleText$1.underline(path.relative(cwd, filepath))}`);
