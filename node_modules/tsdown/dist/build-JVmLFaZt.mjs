...
import { d as toArray, i as importWithError, n as createConcurrencyExecutor, r as debounce, s as pkgExists, t as styleText$1, u as slash } from "./style-DiMP8pYv.mjs";
import { o as globalLogger, r as createSuppressWarnings, t as LogLevels } from "./logger-D9XFzQl9.mjs";
import { a as getPackageType, c as isGlobEntry, d as cleanOutDir, i as loadConfigFile, l as toObjectEntry, n as mergeUserOptions, o as isInCI, r as resolveUserConfig, s as writeExports, t as mergeConfig, u as cleanChunks } from "./options-Bo13C9lJ.mjs";
import { r as getShims, t as DepsPlugin } from "./deps-DCK8g4G9.mjs";
import { a as buildExe } from "./target-DgdLrXba.mjs";
import { n as version } from "./debug-CpC4hV25.mjs";
import { a as ReportPlugin, i as ShebangPlugin, n as endsWithConfig, o as NodeProtocolPlugin, r as addOutDirToChunks, s as copy, t as WatchPlugin } from "./watch-UrMX4y5g.mjs";
...
async function resolveWorkspace(config, inlineConfig, context, rootDeps) {
...
async function resolveConfig(inlineConfig, watch) {
	...
	const { configs: rootConfigs, deps: rootDeps } = await loadConfigFile(inlineConfig, context);
	const globalDeps = new Set(rootDeps);
	const runBuild = createConcurrencyExecutor(inlineConfig.concurrency);
	const configs = (await Promise.all(rootConfigs.map(async (rootConfig) => {
		const { configs: workspaceConfigs, deps: workspaceDeps } = await resolveWorkspace(rootConfig, inlineConfig, context, rootDeps);
...
const label$1 = styleText$1.dim(`[attw]`);
const problemFlags = {
...
async function attw(options, tarball) {
	...
	const { profile = "esm-only", level = "warn", ignoreRules = [], ...attwOptions } = options.attw;
	...
	else options.logger.success(options.nameLabel, label$1, "No problems found", styleText$1.dim(`(${Math.round(performance.now() - t)}ms)`));
}
/**
...
const label = styleText$1.dim(`[publint]`);
async function publint(options, tarball) {
	...
	if (!messages.length) {
		options.logger.success(options.nameLabel, label, "No issues found", styleText$1.dim(`(${Math.round(performance.now() - t)}ms)`));
		return;
	}
...
async function packTarball(packageJsonPath) {
	...
	const { detect } = await import("./detect-Bg-d8iUV.mjs");
	try {
...
async function pack(dir, pm, destination, ignoreScripts) {
	...
	if (destination) switch (pm.agent) {
		...
		case "bun":
			...
			break;
		default: args.push("--pack-destination", destination);
	}
	if (ignoreScripts) switch (pm.agent) {
		...
		default: args.push("--ignore-scripts");
	}
...
async function resolveInputOptions(config, format, configDeps, bundle, cjsDts, isDualFormat) {
	...
	if (config.pkg || config.deps.neverBundle === true || config.deps.dts.neverBundle === true) plugins.push(DepsPlugin(config, bundle));
	if (dts) {
	...
	async function onInput(input) {
		...
		if (input === "h") {
			...
			for (const shortcut of SHORTCUTS) {
				...
				globalLogger.info(styleText$1.dim(`  press `) + styleText$1.bold(`${shortcut.key} + enter`) + styleText$1.dim(` to ${shortcut.description}`));
			}
			return;
		}
...
async function build$1(inlineConfig = {}) {
	...
	const handlePromise = buildWithConfigs(configs, deps, () => build$1(inlineConfig));
	isConfigResolved = true;
	return await handlePromise;
	async function restart() {
		if (!isConfigResolved) throw new Error("`watch.restart` cannot be called before config resolution is complete.");
		return (await handlePromise).watch.restart();
	}
	async function close() {
		if (!isConfigResolved) throw new Error("`watch.close` cannot be called before config resolution is complete.");
...
* @private
*/
async function buildWithConfigs(configs, configDeps, rebuild) {
	...
	function assertWatchMode() {
		if (!hasWatchConfig) throw new Error("`watch` is only available in watch mode.");
	}
	async function close() {
		assertWatchMode();
	...
	async function restart() {
		assertWatchMode();
		if (restarted) throw new Error("`watch.restart` can only be called once per handle.");
		restarted = true;
		await close();
		clearRequireCache();
		return rebuild();
	...
	const bundles = await Promise.all(configs.map((options) => {
		return buildSingle(options, configDeps, options.pkg ? configChunksByPkg[options.pkg.packageJsonPath].formats.size > 1 : true, clean, () => {
			if (restarted) return;
	...
	if (hasWatchConfig) {
		...
		for (const bundle of bundles) disposeCbs.push(bundle[Symbol.asyncDispose]);
...
async function buildSingle(config, configDeps, isDualFormat, clean, restart, done) {
	...
	if (!watch$1) {
		logger.success(config.nameLabel, `Build complete in ${styleText$1.green(`${Math.round(performance.now() - startTime)}ms`)}`);
		await postBuild();
	}
	return bundle;
			...
			switch (event.code) {
				...
				case "BUNDLE_START":
					if (changedFile.length) {
						...
						logger.info(`Found ${styleText$1.bold(changedFile.join(", "))} changed, rebuilding...`);
					}
	...
	async function initBuildOptions() {
		...
		if (format === "cjs" && dts) configs.push(await getBuildOptions(config, format, configDeps, bundle, true, isDualFormat));
		return configs;
	}
