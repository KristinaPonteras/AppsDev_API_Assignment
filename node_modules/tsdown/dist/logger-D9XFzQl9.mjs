import { d as toArray, o as noop, t as styleText } from "./style-DiMP8pYv.mjs";
...
function getNameLabel(styleText, name) {
	if (!name) return void 0;
	return styleText(`[${name}]`);
}
function prettyFormat(format) {
	const formatColor = format === "es" ? styleText.blue : format === "cjs" ? styleText.yellow : noop;
	let formatText;
	switch (format) {
		case "es":
			formatText = "ESM";
			break;
		default: formatText = format.toUpperCase();
	}
...
function generateColor(name = "default") {
	...
	let color;
	if (name === "default") color = styleText.blue;
	else {
		let hash = 0;
		for (let i = 0; i < name.length; i++) hash = name.charCodeAt(i) + ((hash << 5) - hash);
		const hue = hash % 360;
		color = styleText[rgbToHex(...hslToRgb(hue, 35, 55))];
	}
