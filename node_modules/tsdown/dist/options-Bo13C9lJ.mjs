...
import { a as matchPattern, c as resolveComma, d as toArray, i as importWithError, l as resolveRegex, n as createConcurrencyExecutor, s as pkgExists, t as styleText$1, u as slash } from "./style-DiMP8pYv.mjs";
import { a as getNameLabel, i as generateColor, n as createLogger, o as globalLogger } from "./logger-D9XFzQl9.mjs";
import { n as resolveDepsConfig } from "./deps-DCK8g4G9.mjs";
import { n as resolveTarget, o as validateSea } from "./target-DgdLrXba.mjs";
	...
	for (const [format, chunksByFormat] of Object.entries(chunks)) {
		...
		for (const chunk of filteredChunks) {
			...
			if (isIndex) {
				name = ".";
				if (format === "cjs") {
					if (isDts) cjsTypes = distFile;
					else main = distFile;
				} else if (format === "es") {
					if (isDts) esmTypes = distFile;
...
async function resolveTsconfig(logger, tsconfig, cwd, color, nameLabel) {
	...
	if (tsconfig !== false) {
		if (tsconfig === true || tsconfig == null) {
			...
			if (original && !tsconfig) logger.warn(`No tsconfig found in ${styleText$1.blue(cwd)}`);
		} else {
			...
			if (stat?.isFile()) tsconfig = tsconfigPath;
				...
				if (!tsconfig) logger.warn(`No tsconfig found in ${styleText$1.blue(tsconfigPath)}`);
			} else {
				tsconfig = findTsconfig(cwd, tsconfig);
				if (!tsconfig) logger.warn(`tsconfig ${styleText$1.blue(original)} doesn't exist`);
			}
		}
...
async function loadViteConfig(prefix, cwd, configLoader) {
	...
	globalLogger.info(`Using Vite config: ${styleText$1.underline(source)}`);
...
async function loadConfigFile(inlineConfig, context, workspace) {
	...
	if (result) {
		...
		globalLogger.info(`config file: ${styleText$1.underline(file)}`, loader === "native" ? "" : `(${loader})`);
		exported = await exported;
		if (typeof exported === "function") exported = await exported(inlineConfig, context);
	}
...
async function resolveUserConfig(userConfig, inlineConfig, configDeps, runBuild = createConcurrencyExecutor()) {
	...
	let { entry, format, plugins = [], clean = true, logLevel = "info", failOnWarn = false, suppressWarnings, customLogger, treeshake = true, platform = "node", outDir = "dist", sourcemap = false, dts, unused = false, watch = false, ignoreWatch, shims = false, publint = false, attw = false, fromVite, alias, tsconfig, report = true, target, env = {}, envFile, envPrefix = "TSDOWN_", copy, hash = true, cwd = process.cwd(), name, workspace, exports = false, unbundle = false, root, nodeProtocol = false, cjsDefault = true, globImport = true, css, outExtensions, fixedExtension = platform === "node", devtools = false, write = true, exe = false } = userConfig;
...
//#endregion
export { getPackageType as a, isGlobEntry as c, cleanOutDir as d, loadConfigFile as i, toObjectEntry as l, mergeUserOptions as n, isInCI as o, resolveUserConfig as r, writeExports as s, mergeConfig as t, cleanChunks as u };
